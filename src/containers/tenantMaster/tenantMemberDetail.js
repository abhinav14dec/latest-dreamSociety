...
import {
  viewMember,
  deleteTenantMember,
  deleteSelectedTenantMember,
  editTenantMember,
  addNewTenantDetail,
  rfid,
  validOnChangeContact,
  validOnChangeEmail,
  getFlats
} from "../../actions/tenantMasterAction";
...
class TenantMemberDetail extends Component {
  ...
  edit = (
    ...
    this.setState({
      ...
      defFlat: flatId,
      flatNo,
      editTenantMember: !this.state.editTenantMember
    });
  };
    ...
    return function(x) {
      if (x) {
        return (
          ...
          x.relation_master.relationName
            .toLowerCase()
            .indexOf(search.toLowerCase()) !== -1 ||
          !search
        );
          ...
          if (!item) {
            ...
            return (
              <tr key={item.memberId}>
                ...
                <td>{item.contact}</td>
                <td>
                  {item.relation_master
                    ? item.relation_master.relationName
                    : ""}
                </td>
                <td>
                  {item.flat_detail_master
                    ? item.flat_detail_master.flatNo
                    : ""}
                </td>
                ...
                <td>
                  <Button
                    ...
                    onClick={this.edit.bind(
                      ...
                      item.relationId,
                      item.memberId,
                      item.rfid_master ? item.rfid_master.rfidId : "",
                      item.rfid_master ? item.rfid_master.rfid : "",
                      item.flatDetailId,
                      item.flat_detail_master
                        ? item.flat_detail_master.flatNo
                        : ""
                  ...
                  <Button
                    color="danger"
                    onClick={this.deleteMemberSelected.bind(
                      this,
                      item.memberId
                    )}
                  >
  ...
  toggleTenantMemberForm() {
    this.setState({
      addTenantMember: !this.state.addTenantMember,
      state: "",
      errors: {}
    });
  }
  ...
  getRelationOption = ({ relationResult }) => {
    if (relationResult && relationResult.relation) {
      return relationResult.relation.map(item => {
        return (
          <option value={item.relationId} key={item.relationId}>
            {item.relationName}
          </option>
        );
      });
  ...
  deleteSelected(ids) {
    ...
    this.props
      .deleteSelectedTenantMember(ids)
  ...
  updateTenantMember = e => {
    ...
    if (aadhaarNumber === "")
      ...
      errors.aadhaarNumber = `Aadhaar number should be of 12 digits.`;
    if (flatDetailId === "") errors.flatDetailId = `Please select Flat.`;
    this.setState({ errors });
    const isValid = Object.keys(errors).length === 0;
    if (
      isValid &&
      this.state.emailChangeErr === "" &&
      this.state.validChangeContactErr === ""
    ) {
  ...
  addNewTenantMember(
    ...
    if (contact) {
      if (contact.length !== 10)
        errors.contact = `Contact should be of 10 digit.`;
    }
    this.setState({ errors });
    const isValid = Object.keys(errors).length === 0;
    if (
  ...
  loadDetailAfterAdd = () => {
    this.props
      .viewMember(this.state.tenantId)
      .then(() =>
        this.setState({ loadingAfterAdd: false, addTenantMember: false })
      )
  ...
  rfidOptions = ({ getRFID }) => {
    if (getRFID && getRFID.rfids) {
      return getRFID.rfids.map(item => {
        return { ...item, name: "rfid", label: item.rfid, value: item.rfidId };
      });
    }
    return [];
  };
  ...
  editFlat = () => {
    if (!!document.getElementById("isFlatChecked").checked) {
      ...
      this.setState({
        flatDetailId: this.state.defFlat,
        defaultFlat: true,
        editFlat: false
      });
    }
  ...
  emailChange = e => {
    ...
    this.props
      .validOnChangeEmail(e.target.value)
      .then(res => res)
      .catch(err => {
  ...
  contactChange = e => {
    ...
    if (!!this.state.errors[e.target.name]) {
      ...
      this.props
        .validOnChangeContact(e.target.value)
        .then(res => res)
        .catch(err => {
  ...
  render() {
    let tableData = (
      <Table className="table table-bordered">
        <thead>
          <tr>
            ...
            <th
              ...
              onClick={() => {
                this.setState(state => {
                  return { sortVal: !state.sortVal, filterName: "firstName" };
                });
              }}
            >
              Member Name
        ...
        <tbody>{this.fetchMemberDetails(this.props.tenantReducer)}</tbody>
      </Table>
    );
    let addForm = (
      <div>
        <FormGroup>
          ...
          <Input
            name="firstName"
            onChange={this.addNewMemberChange}
        ...
        <FormGroup>
          ...
          />
          <div>
            {<span className="error">{this.state.validChangeContactErr}</span>}
          </div>
          <br />
        ...
        <FormGroup>
          <Row md={12}>
            <Col md={6}>
              ...
              <Select
                name="relationId"
                options={this.getRelationList(this.props.relationList)}
        ...
        <FormGroup>
          <Button
            className="mr-2"
            color="primary"
            onClick={this.addNewTenantMember.bind(
              this,
              this.state.firstName,
              this.state.lastName,
              this.state.memberDob,
          ...
          <Button
            color="danger"
            onClick={this.toggleTenantMemberForm.bind(this)}
          >
            Cancel
          </Button>
        </FormGroup>
      ...
      <div>
        ...
        <FormGroup>
          <Row md={12}>
            ...
            {this.state.defaultRFID ? (
              <Col md={6} style={{ padding: "34px" }}>
                <span style={{ fontWeight: "bold" }}>
                  Do you want to edit your RFID?
                </span>
                <Input
                  type="checkbox"
                  onChange={this.editRFID}
                  name="isRfidChecked"
                  id="isRfidChecked"
                  className="ml-3"
                />
        ...
        <FormGroup>
          <Row md={12}>
            ...
            {this.state.defaultFlat ? (
              <Col md={6} style={{ padding: "34px" }}>
                <span style={{ fontWeight: "bold" }}>
                  Do you want to edit your Flat?
                </span>
                <Input
                  type="checkbox"
                  onChange={this.editFlat}
                  name="isFlatChecked"
                  id="isFlatChecked"
                  className="ml-3"
                />
        ...
        <FormGroup>
          <Button
            className="mr-2"
            color="primary"
            onClick={this.updateTenantMember}
          >
            Save
          </Button>
          <Button color="danger" onClick={this.toggleEditTenant.bind(this)}>
            Cancel
          </Button>
        </FormGroup>
      </div>
        ...
        <div className="w3-container w3-margin-top w3-responsive">
          ...
          <div className="top-details">
            ...
            <div>
              ...
              <Button color="primary" onClick={this.route}>
                View Tenant Detail
              </Button>
            </div>
          </div>
          ...
          <Modal
            isOpen={this.state.addTenantMember}
            toggle={this.toggleTenantMemberForm.bind(this)}
          >
            <ModalHeader toggle={this.toggleTenantMemberForm.bind(this)}>
              Add New Members
            </ModalHeader>
            <ModalBody>
          ...
          <Modal
            isOpen={this.state.editTenantMember}
            toggle={this.toggleEditTenant.bind(this)}
          >
            <ModalHeader toggle={this.toggleEditTenant.bind(this)}>
              Edit Member Detail
            </ModalHeader>
            <ModalBody>
