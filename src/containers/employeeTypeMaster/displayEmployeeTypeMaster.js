...
class DisplayEmployeeTypeMaster extends Component {
  ...
  deleteEmployee(employeedetailId) {
    ...
    this.props
      .deleteEmployee(employeedetailId, isActive)
  ...
  getEmpType({ employeeType }) {
    if (employeeType) {
      return employeeType.employeeType.map(item => {
        return (
  ...
  getEmpWorkType({ employeeWorkType }) {
    if (employeeWorkType) {
      return employeeWorkType.employeeWorkType.map(item => {
        return (
  ...
  searchFilter(search) {
    return function(x) {
      return (
        x.serviceType.toLowerCase().includes(search.toLowerCase()) ||
        x.employee_work_type_master.employeeWorkType
          .toLowerCase()
          .includes(search.toLowerCase()) ||
        x.employee_type_master.employeeType
          .toLowerCase()
          .includes(search.toLowerCase()) ||
        !search
      );
  ...
  getEmployee({ getEmployee }) {
    if (getEmployee && getEmployee.employeeDetail) {
      return getEmployee.employeeDetail
        .sort((item1, item2) => {
        ...
        .map((item, index) => {
          return (
            <tr key={item.employeeDetailId}>
                ...
                <input
                  ...
                  onChange={(e, i) => {
                    ...
                    if (!e.target.checked) {
                      ...
                      this.setState({
                        ids: [...this.state.ids, employeeDetailId]
                      });
                      if (this.state.ids.length >= 0) {
              ...
              <td>
                {item.employee_work_type_master
                  ? item.employee_work_type_master.employeeWorkType
                  : ""}
              </td>
              <td>
                {item.employee_type_master
  ...
  render() {
    ...
    tableData = (
      <Table>
        <thead>
          <tr>
            ...
            <th
              onClick={() => {
                this.setState(state => {
                  return { sortVal: !state.sortVal, filterName: "serviceType" };
                });
              }}
            >
              Service Type
              <i class="fa fa-arrows-v" id="sortArrow" aria-hidden="true"></i>
            </th>
        ...
        <tbody>{this.getEmployee(this.props.employeeDetails)}</tbody>
      </Table>
    );
      ...
      <div>
        ...
        <FormGroup>
          ...
          <select
            ...
            value={this.state.editEmployeeData.employeeWorkTypeId}
            onChange={e => {
        ...
        <FormGroup>
          ...
          <select
            id="serviceType"
            className="form-control"
            value={this.state.editEmployeeData.employeeTypeId}
            onChange={e => {
              let { editEmployeeData } = this.state;
        ...
        <Button color="primary" className="mr-2" onClick={this.updateEmployee}>
          Save
        </Button>
        <Button
          color="danger"
