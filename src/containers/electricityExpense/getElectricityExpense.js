...
import {
  getElectricityExpense,
  getRateForElectricityExpense,
  deleteElectricityExpense,
  deleteSelectedElectricityExpense,
  updateElectricityExpense
} from "../../actions/electricityExpense";
...
class GetElectricityExpense extends Component {
  ...
  deleteSelected(ids) {
    ...
    this.props
      .deleteSelectedElectricityExpense(ids)
    ...
    if (expenseDetail && expenseDetail.electricityConsumer) {
      ...
      return expenseDetail.electricityConsumer
        .sort((item1, item2) => {
          ...
          var cmprVal =
            items1 && items2
              ? items1[this.state.filterName].localeCompare(
                  items2[this.state.filterName]
                )
              : "";
            ...
            <tr key={item.electricityConsumerId}>
              <td>
                <input
                  ...
                  name="ids"
                  className="SelectAll"
                  value={item.electricityConsumerId}
                  onChange={e => {
                    ...
                    if (!e.target.checked) {
                      ...
                      this.setState({
                        ids: [...this.state.ids, electricityConsumerId]
                      });
                      if (this.state.ids.length >= 0) {
                        this.setState({ isDisabled: false });
                      }
              ...
              <td>{index + 1}</td>
              <td>
                {item.flat_detail_master.tower_master
                  ? item.flat_detail_master.tower_master.towerName
                  : ""}
              </td>
              <td>
                {item.flat_detail_master.floor_master
                  ? item.flat_detail_master.floor_master.floorName
                  : ""}
              </td>
              <td>
                {item.flat_detail_master ? item.flat_detail_master.flatNo : ""}
              </td>
              ...
              <td>
                ...
                <Button
                  color="danger"
                  className="danger"
                  onClick={this.delete.bind(this, item.electricityConsumerId)}
                >
                  Delete
                </Button>
              </td>
  ...
  delete = electricityConsumerId => {
    ...
    this.props
      .deleteElectricityExpense(electricityConsumerId, isActive)
      .then(() => this.refreshData());
  ...
  searchFilter =search => {
    return function(x) {
      return (
        ...
        x.flat_detail_master.floor_master.floorName
          .toLowerCase()
          .includes(search.toLowerCase()) ||
        x.flat_detail_master.flatNo
          .toLowerCase()
          .includes(search.toLowerCase()) ||
        !search
      );
  ...
  sameSign = () => {
    if (!!document.getElementById("isChecked").checked) {
      ...
      this.setState({
        amountDue: this.state.amountDueInput,
        defaultSign: true,
        editSign: false
      });
    }
      ...
      <div>
        ...
        <FormGroup>
          <Row md={12}>
            {this.state.defaultSign ? (
              <Col md={6}>
                <Input
                  ...
                  disabled
                  maxLength="500"
                  value={this.state.amountDue === true ? "-" : "+"}
                  name="amountDueInput"
            ...
            {this.state.defaultSign ? (
              <Col md={6}>
                <span style={{ fontWeight: "bold" }}>Do you want to edit?</span>
                <Input
                  type="checkbox"
                  onChange={this.sameSign}
                  name="isChecked"
                  id="isChecked"
                  className="ml-3"
                />
