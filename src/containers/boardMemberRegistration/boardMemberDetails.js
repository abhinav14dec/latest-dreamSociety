...
import {
  ...
  getCity,
  getLocation
} from "../../actions/societyMasterAction";
import Select from "react-select";
import {
  getMemberDetails,
  getSocietyId,
  getMemberDesignation,
  updateSocietyMemberDetails,
  deleteSocietyMemberDetail,
...
class BoardMemberDetails extends Component {
  ...
  viewMember(
    ...
    this.setState({
      ...
      IFSCCode,
      accountHolderName,
      viewSocietyMember: !this.state.viewSocietyMember,
      readOnlyPermanent: permanentAddress,
      readOnlyCurrent: currentAddress
    });
  }
  ...
  editMember(
    ...
    this.setState({
      ...
      accountHolderName,
      designationId,
      editSocietyMember: !this.state.editSocietyMember,
      readOnlyPermanent: permanentAddress,
      readOnlyCurrent: currentAddress
    });
  }
    ...
    return function(x) {
      if (x) {
        return (
          ...
          x.designation_master.designationName
            .toLowerCase()
            .indexOf(search.toLowerCase()) !== -1 ||
          x.currentAddress.toLowerCase().indexOf(search.toLowerCase()) !== -1 ||
          x.permanentAddress.toLowerCase().indexOf(search.toLowerCase()) !==
            -1 ||
  ...
  fetchMemberDetails = ({ memberDetails }) => {
    if (memberDetails && memberDetails.societyBoardMember) {
      return memberDetails.societyBoardMember
        .sort((item1, item2) => {
          if (item1 && item2) {
            var cmprVal = item1[this.state.filterName].localeCompare(
              item2[this.state.filterName]
            );
              ...
              <tr key={item.societyBoardMemberId}>
                <td>
                  <input
                    ...
                    name="ids"
                    className="SelectAll"
                    value={item.societyBoardMemberId}
                    onChange={e => {
                      ...
                      if (!e.target.checked) {
                        ...
                        this.setState({
                          ids: [...this.state.ids, societyBoardMemberId]
                        });
                        if (this.state.ids.length >= 0) {
                ...
                <td>
                  <Button
                    color="success"
                    className="mr-2"
                    onClick={this.viewMember.bind(
                      this,
                      item.firstName,
                      item.lastName,
                      item.designation_master.designationName,
                      item.gender,
                      item.currentAddress,
                ...
                <td>
                  ...
                  <Button
                    color="danger"
                    onClick={this.deleteSocietyMember.bind(
                      this,
                      item.societyBoardMemberId
                    )}
                  >
  ...
  deleteSocietyMember(societyBoardMemberId) {
    ...
    this.props
      .deleteSocietyMemberDetail(societyBoardMemberId)
      .then(() => this.refreshData());
  }

  route = () => {
    return this.props.history.replace(
      "/superDashboard/boardMemberRegistartionForm"
    );
  };
  ...
  deleteSelected = ids => e => {
    ...
    this.setState({ loading: true, isDisabled: true });
    this.props
      .deleteMultipleSocietyMemberDetail(ids)
      .then(() => this.refreshData());
  };
  ...
  toggleEditSocietyMember() {
    this.setState({
      ...
      cityId: "",
      locationId: "",
      permanentAddress: this.state.readOnlyPermanent,
      editCurrent: false,
      userCurrent: false,
      currentCountryId: "",
      currentStateId: "",
  ...
  countryName = ({ countryResult }) => {
    if (countryResult) {
      return countryResult.map(item => {
        return { ...item, label: item.countryName, value: item.countryId };
      });
    }
  };
  ...
  updateCountryAddress = countryName => {
    ...
    this.setState({
      permanentAddress:
        this.state.permanentAddressDefault +
        ", " +
  ...
  cityName = ({ cityResult }) => {
    if (cityResult) {
      return cityResult.map(item => {
        if (item) {
          return { ...item, label: item.cityName, value: item.cityId };
        }
        return null;
      });
    }
  ...
  locationName = ({ locationResult }) => {
    if (locationResult) {
      return locationResult.map(item => {
        return { ...item, label: item.locationName, value: item.locationId };
      });
    }
  };
  ...
  emailChange = e => {
    ...
    if (
      ...
      this.setState({ emailValidError: "Invalid Email." });
    }
    if (!!this.state.errors[e.target.name]) {
      let errors = Object.assign({}, this.state.errors);
      delete errors[e.target.name];
  ...
  update = e => {
    ...
    if (contactNumber === "") {
      errors.contactNumber = `Contact can't be empty.`;
    } else if (contactNumber.length !== 10)
      errors.contactNumber = "Contact length should be of 10.";
    if (email === "") {
    ...
    if (panCardNumber === "") {
      errors.panCardNumber = `Pan Card Number can't be empty.`;
    } else if (panCardNumber.length !== 10) {
      errors.panCardNumber = `Pan length should be of 10.`;
    }
    if (dob === "") {
    ...
    if (!!document.getElementById("isChecked").checked) {
      if (pin1 === "") errors.pin1 = `Pin/Zip code can't be empty.`;
        ...
        errors.pin1 = `Pin/Zip code should be of 5 digits atleast.`;
    }
    if (!!document.getElementById("isCurrentChecked").checked) {
      if (pin === "") errors.pin = `Pin/Zip code can't be empty.`;
      else if (pin.length < 5)
        errors.pin = `Pin/Zip code should be of 5 digits atleast.`;
    }
    ...
    if (isValid && this.state.emailValidError === "") {
      ...
      this.props
        ...
        .catch(err => {
          this.setState({
            modalLoading: false,
            contactServerError: err.response.data.messageContactErr,
  ...
  editPermanentAddress = () => {
    if (!!document.getElementById("isChecked").checked) {
      ...
      this.setState({
        ...
        permanentAddress: this.state.readOnlyPermanent,
        userPermanent: false,
        countryId: this.state.readOnlyCountryId,
        stateId: this.state.readOnlyStateId,
        cityId: this.state.readOnlyCityId,
  ...
  permanentAddressChange = e => {
    if (!!this.state.errors[e.target.name]) {
      ...
      delete errors[e.target.name];
      this.setState({
        permanentAddressDefault: e.target.value,
        permanentAddress:
          e.target.value +
          (this.state.locationName
            ? ", " + this.state.locationName + ", "
  ...
  updateCountryCurrent = currentCountry => {
    ...
    this.setState({
      currentAddress:
        this.state.currentAddressDefault +
        ", " +
        (this.state.currentLocation
          ? ", " + this.state.currentLocation + ", "
          : ", ") +
  ...
  currentAddressChange = e => {
    if (!!this.state.errors[e.target.name]) {
      ...
      delete errors[e.target.name];
      this.setState({
        currentAddressDefault: e.target.value,
        currentAddress:
          e.target.value +
          (this.state.currentLocation
            ? ", " + this.state.currentLocation + ", "
  ...
  currentAddressIsChecked = () => {
    if (!!document.getElementById("isCurrentChecked").checked) {
      ...
      this.setState({
        editCurrent: false,
        currentAddress: this.state.readOnlyCurrent,
        userCurrent: false
      });
    }
  };
  ...
  render() {
    let tableData = (
      <Table className="table table-bordered">
        ...
        <tbody>{this.fetchMemberDetails(this.props.boardMemberReducer)}</tbody>
      </Table>
    );
    ...
    let modalData = (
      <div>
        ...
        <FormGroup>
          ...
          <Label
            htmlFor="Gender1"
            style={{ paddingRight: "35px", paddingLeft: "20px" }}
          >
            Male
          </Label>
          <span>
          ...
          <span>
            <Input
              ...
              checked={this.state.Female === this.state.gender ? true : false}
            />
          </span>
          ...
          <span>
            <Input
              ...
              checked={this.state.Other === this.state.gender ? true : false}
            />
          </span>
        </FormGroup>
        <FormGroup>
          <Row md={12}>
            <Col md={6}>
              ...
              {this.state.contactServerError ? (
                <span className="error">{this.state.contactServerError}</span>
              ) : null}
            </Col>
            <Col md={6}>
              <Label>Email</Label>
              ...
              />
              {this.state.emailServerError ? (
                <span className="error">{this.state.emailServerError}</span>
              ) : null}
              <span>
                <br />
              </span>
            ...
            {this.state.editPermanent ? (
              <Col md={12} style={{ textAlign: "center" }}>
                <span style={{ fontWeight: "600" }}>
                  Do you want to edit permanent address?
                </span>
                <Input
                  type="checkbox"
                  onChange={this.editPermanentAddress}
                  name="isChecked"
                  id="isChecked"
                  className="ml-3"
                />
              ...
              <Col md={6} style={{ paddingTop: "44px" }}>
                <span style={{ fontWeight: "600" }}>
                  Do you want to edit permanent
                  <br /> address?
                </span>
                <Input
                  type="checkbox"
              ...
              <Row md={12}>
                <Col md={3}>
                  ...
                  <Select
                    ...
                    options={this.countryName(this.props.societyReducer)}
                ...
                <Col md={3}>
                  ...
                  <Select
                    placeholder={<DefaultSelect />}
                    options={this.stateName(this.props.societyReducer)}
                ...
                <Col md={3}>
                  ...
                  <Select
                    placeholder={<DefaultSelect />}
                    options={this.cityName(this.props.societyReducer)}
                ...
                <Col md={3}>
                  ...
                  <Select
                    placeholder={<DefaultSelect />}
                    options={this.locationName(this.props.societyReducer)}
                    onChange={this.onChangeLocation.bind(
                      this,
                      "locationName",
                      "locationId"
                    )}
                ...
                <Col md={8}>
                  ...
                  />
                  {
                    <span className="error">
                      {this.state.errors.permanentAddressDefault}
                    </span>
                  }
                </Col>
              </Row>
        ...
        <FormGroup>
          <Row md={12}>
            ...
            {!this.state.editCurrent ? (
              <Col md={6} style={{ paddingTop: "44px" }}>
                <span style={{ fontWeight: "600" }}>
                  Do you want to edit current address?
                </span>
                <Input
                  type="checkbox"
                  name="isCurrentChecked"
                  id="isCurrentChecked"
                  onChange={this.currentAddressIsChecked}
                  className="ml-3"
                />
              </Col>
            ) : (
            ...
            <FormGroup>
              <Row md={12}>
                <Col md={3}>
                  ...
                  <Select
                    placeholder={<DefaultSelect />}
                    options={this.countryName1(this.props.societyReducer)}
                    onChange={this.countryChange.bind(
                      this,
                      "currentCountry",
                      "currentCountryId"
                    )}
                ...
                <Col md={3}>
                  ...
                  <Select
                    placeholder={<DefaultSelect />}
                    options={this.stateName1(this.props.societyReducer)}
                    onChange={this.stateChange.bind(
                      this,
                      "currentState",
                      "currentStateId"
                    )}
                ...
                <Col md={3}>
                  ...
                  <Select
                    placeholder={<DefaultSelect />}
                    options={this.cityName1(this.props.societyReducer)}
                    onChange={this.cityChange.bind(
                      this,
                      "currentCity",
                      "currentCityId"
                    )}
                ...
                <Col md={3}>
                  ...
                  <Select
                    placeholder={<DefaultSelect />}
                    options={this.locationName1(this.props.societyReducer)}
                    onChange={this.locationChange.bind(
                      this,
                      "currentLocation",
                      "currentLocationId"
                    )}
            ...
            <FormGroup>
              <Row md={12}>
                ...
                <Col md={8}>
                  ...
                  {
                    <span className="error">
                      {this.state.errors.currentAddressDefault}
                    </span>
                  }
                </Col>
              </Row>
        ...
        <FormGroup>
          ...
          <Button
            color="danger"
            onClick={this.toggleEditSocietyMember.bind(this)}
          >
            Cancel
          </Button>
        </FormGroup>
    ...
    let deleteSelectedButton = (
      <Button
        ...
        color="danger"
        className="mb-3"
        onClick={this.deleteSelected(this.state.ids)}
      >
        Delete Selected
      </Button>
    );
    ...
    let memberData = (
      <div>
        ...
        <FormGroup>
          <Button
            color="primary"
            onClick={this.toggleViewtSocietyMember.bind(this)}
          >
            Cancel
          </Button>
        </FormGroup>
      ...
      <UI onClick={this.logout} change={this.changePassword}>
        <div className="w3-container w3-margin-top w3-responsive">
          ...
          <div className="top-details">
            ...
            <Button onClick={this.route} color="primary">
              Add Board Member
            </Button>
          </div>
          <Modal
            isOpen={this.state.editSocietyMember}
            toggle={this.toggleEditSocietyMember.bind(this)}
          >
            <ModalHeader toggle={this.toggleEditSocietyMember.bind(this)}>
              Edit Board Member Details
            </ModalHeader>
            <ModalBody>
          ...
          <Modal
            isOpen={this.state.viewSocietyMember}
            toggle={this.toggleViewtSocietyMember.bind(this)}
          >
            <ModalHeader toggle={this.toggleViewtSocietyMember.bind(this)}>
              {" "}
              Board Member Details
            </ModalHeader>
            <ModalBody>{memberData}</ModalBody>
          </Modal>
          <SearchFilter
            type="text"
