...
class BoardMemberRegistrationForm extends Component {
    ...
    submit = (e) => {
        ...
        if (isValid && this.state.emailValidError === '') {
            ...
            this.props.addMemberDetails(this.state)
                ...
                .catch(err => {
                    this.setState({
                        emailServerError: err.response.data.messageEmailErr, userNameServerError: err.response.data.messageUsernameErr,
    ...
    permanentAddressChange = (e) => {
        ...
        if (!!document.getElementById('isChecked').checked) {
            this.setState({
                currentAddress: e.target.value + (this.state.locationName ? (', ' + this.state.locationName + ', ') : ', ') +
                    this.state.cityName + ', ' + this.state.stateName + ', ' + this.state.countryName + ', ' + 'Pin/Zip code: ' + this.state.pin1
            })
        }
    }
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        getCountry, getState, getCity, addMemberDetails,
        getMemberDesignation, getMemberDetails, getSocietyId, getLocation
    }, dispatch);
}
