  ...
  searchFilter = search => {
    return function(x) {
      return (
        ...
        x.flats[0].tower_master.towerName
          .toLowerCase()
          .includes(search.toLowerCase()) ||
        x.flats[0].floor_master.floorName
          .toLowerCase()
          .includes(search.toLowerCase()) ||
        !search
      );
        ...
        .map((item, index) => {
          return (
            <tr key={item.userId}>
              ...
              <td>
                {" "}
                {item.flats[0] ? item.flats[0].tower_master.towerName : ""}
              </td>
              <td>
                {" "}
                {item.flats[0] ? item.flats[0].floor_master.floorName : ""}
              </td>
              ...
              <td>
                <Input
                  ...
                  name="flatDetailId"
                  onChange={this.onChangeInput.bind(this, item.userId)}
                >
                  <DefaultSelect />
                  {this.getDropdownForFlats(
                    this.props.fingerprintReducer,
                    item.userId
                  )}
                </Input>
              ...
              <td>
                <Button
                  color="success"
                  className="mr-2"
                  disabled={item.userId !== this.state.selectedUser}
                  onClick={this.getMachineComponent.bind(
                    this,
                    item.userId,
                    this.state.flatDetailId,
                    this.state.type
...
export default connect(
  mapStateToProps,
  { getFingerprintData, getMachineData }
)(FingerPrint);
