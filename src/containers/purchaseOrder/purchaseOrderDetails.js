...
class PurchaseOrderDetails extends Component {
  ...
  refreshData() {
    ...
    this.props
      .getPurchaseOrder()
      .then(() =>
        this.setState({ loading: false, modalLoading: false, modal: false })
      );
    ...
    this.setState(
      {
        ...
        data: { ...this.state.data, purchaseOrderSubType: value },
  ...
  onRateChangeHandler = e => {
    this.setState(
      {
        data: { ...this.state.data, rate: e.target.value }
      },
      function() {}
    );
  };
  ...
  searchFilter(search) {
    return function(x) {
      return (
        x.vendor_master.firstName
          .toLowerCase()
          .includes(search.toLowerCase()) ||
        x.issuedBy.toLowerCase().includes(search.toLowerCase()) ||
        x.purchaseOrderId.toString().includes(search.toLowerCase()) ||
        !search
      );
    };
  }
  ...
  downloadPdf = purchaseOrderId => {
    this.props.pdf(purchaseOrderId).then(res => {
      this.setState({
  ...
  delete = purchaseOrderId => {
    ...
    if (window.confirm("Are You Sure ?")) {
      this.props.removePurchaseOrder(purchaseOrderId).then(() => {
        this.props
          .getPurchaseOrder()
  ...
  renderList = ({ getpurchaseOrder }) => {
    if (getpurchaseOrder && getpurchaseOrder.purchaseOrder) {
      return getpurchaseOrder.purchaseOrder
        .sort((item1, item2) => {
          let cmpValue = item1.vendor_master[
            this.state.filterName
          ].localeCompare(item2.vendor_master[this.state.filterName]);
        ...
        .map((item, index) => {
          return (
            <tr key={index}>
                ...
                <input
                  ...
                  onChange={(e, i) => {
                    ...
                    if (!e.target.checked) {
                      ...
                      this.setState({
                        ids: [...this.state.ids, purchaseOrderId]
                      });
                      if (this.state.ids.length >= 0) {
                        this.setState({ isDisabled: false });
                      }
              ...
              <td>
                {item.vendor_master.firstName +
                  " " +
                  item.vendor_master.lastName}
              </td>
              <td>{item.vendor_master.contact}</td>
              <td>
                <button
                  className="btn btn-success mr-2"
                  onClick={this.downloadPdf.bind(this, item.purchaseOrderId)}
                >
                  View
                </button>
              </td>
              <td style={{ textAlign: "center" }}>
                <button
                  ...
                  onClick={this.viewAssets.bind(this, item.purchaseOrderId)}
                >
                  View
                </button>
              </td>
              <td style={{ textAlign: "center" }}>
                <button
                  className="btn btn-success"
                  onClick={this.viewServices.bind(this, item.purchaseOrderId)}
                >
                  View
                </button>
              </td>
              <td style={{ textAlign: "center" }}>
                <button
                  className="btn btn-success mr-2"
                  onClick={this.editPurchaseOrder.bind(
                    this,
                ...
                <button
                  ...
                  onClick={this.delete.bind(this, item.purchaseOrderId)}
                >
                  Delete
                </button>
              </td>
  ...
  getAsset = ({ getAssetsType }) => {
    if (getAssetsType && getAssetsType.assetsType) {
      return getAssetsType.assetsType.map(item => {
        return (
          <option
            value={item.asset_master.assetId}
            key={item.asset_master.assetId}
          >
  ...
  getAssetType = ({ assetTypeData }) => {
    if (assetTypeData && assetTypeData.assetsType) {
      return assetTypeData.assetsType.map(item => {
        return (
    ...
    this.setState(
      {
        ...
        data: { ...this.state.data, purchaseOrderName: e.target.value }
      },
      function() {}
    );
  };
  ...
  render() {
          ...
          <Row>
            ...
            <Col md={4}>
              ...
              <Input
                type="text"
                name={`serviceRate${i}`}
                onChange={this.onServiceRateChangeHandler}
                value={this.state["serviceRate" + i]}
              />
            </Col>
            <Col md={4}>
          ...
          <Row>
            <Col md={4}>
              ...
              <Input
                type="text"
                name={`person${i}`}
                onChange={this.onPersonChangeHandler.bind(this, i)}
            ...
            <Col md={4}>
              ...
              <Input
                type="text"
                name={`serviceAmount${i}`}
                onChange={this.onChangeHandler}
                value={this.state[`serviceAmount${i}`]}
                readOnly
              />
            </Col>
            <Col md={4}>
          ...
          <Row>
            ...
            <Col md={6}>
              ...
              <Input
                type="text"
                name={`assetRate${i}`}
                onChange={this.onRateChangeHandler}
                value={this.state["assetRate" + i]}
              />
            </Col>
          </Row>
          <Row>
            <Col md={4}>
              ...
              <Input
                type="text"
                name={`assetQuantity${i}`}
                onChange={this.onQuantityChangeHandler.bind(this, i)}
                value={this.state["assetQuantity" + i]}
              />
            </Col>
            <Col md={4}>
              <Label>Amount</Label>
              <Input
                ...
                readOnly
                onChange={this.onChangeHandler}
                value={this.state[`assetAmount${i}`]}
              />
            </Col>
          </Row>
        </FormGroup>
    ...
    tableData = (
      <Table className="table table-bordered">
        ...
        <tbody>{this.renderList(this.props.purchase)}</tbody>
      </Table>
    );
        ...
        <FormGroup>
          <Row>
            ...
            <Col md={6}>
              ...
              <Input
                type="date"
                name="expDateOfDelievery"
                min={this.minDate()}
                onChange={this.onChangeHandler}
                value={this.state.expDateOfDelievery}
              ></Input>
            </Col>
          </Row>
        </FormGroup>
          ...
          <div className="w3-container w3-margin-top w3-responsive">
            ...
            <div className="top-details">
              ...
              <Button
                color="primary"
                onClick={() =>
                  this.props.history.push("/superDashboard/purchaseOrder")
                }
                id="addOwner"
              >
                Add Purchase Order
            ...
            <Modal
              isOpen={this.state.pdfOpen}
              toggle={this.toggleEmployeeModal.bind(this)}
            >
              <ModalHeader toggle={this.toggleEmployeeModal.bind(this)}>
                Purchase Details
              </ModalHeader>
              <ModalBody>
                {pdfDisplay}
                <Button
                  color="primary"
                  onClick={this.toggleEmployeeModal.bind(this)}
                >
                  Cancel
                </Button>
              </ModalBody>
            ...
            <Modal isOpen={this.state.modal} toggle={this.editModal.bind(this)}>
              <ModalHeader toggle={this.editModal.bind(this)}>
