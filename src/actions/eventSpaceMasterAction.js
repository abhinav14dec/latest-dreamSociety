...
import {
    URN, ADD_EVENT_SPACE_DETAILS, FETCH_EVENT_SPACE_DETAILS, DELETE_MULTIPLE_EVENT_SPACE_MASTER_DETAILS,
    UPDATE_EVENT_SPACE
...
export function updateEventSpace(eventSpaceId, spaceName, capacity, spaceType, sizeId, area, from, to, price, description) {
    ...
    const request = axios.put(`${URN}/eventSpaceMaster/` + eventSpaceId, {
        spaceName, capacity, spaceType, sizeId, area, from, to, price, description
    }, { headers: authHeader() })
        .then(response => response.data)
